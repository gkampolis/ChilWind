...
None of the methods managed to capture the “ramp” event at the start of the forecasting period, suggesting that a sub-hourly approach may be needed. Furthermore, the Drift method performed marginally better than Persistence. The lowest MAE in the training set was achieved by STL approaches which include Annual and Diurnal seasonality. They subsequently performed very poorly for forecasting, suggesting that good fit in the training set is not an indication of performance and that including annual seasonality may lead to overfitting. Data pre-processing/transformations hindered the forecasting ability of the models universally. 

Most of the literature focuses on ARIMA methods (as for many cases ARIMA is considered a generalization of ETS but is not always strictly true, see [Hyndman and Athanssopoulos](https://otexts.org/fpp2/arima-ets.html)), but the ETS performed significantly better and is characterized by a very low computation cost even for long time series, making it a good recommendation to try it out by other forecasters in future work. In particular, the low computation cost enables the method to be a good stand-in replacement for simple persistence to cover short-term, when combined with NWP for predictions further ahead. Finally, as the ETS results showed promise after a few hours of forecasting horizon in this case, the model's output could potentially also serve as input to a neural network approach which would combine them with NWP output to further minimize error.
